import { NPM_BETA_URL } from '../lib/cosntants'
...
function LeftPanel({ isCollapsed }: ResizableSidebarProps) {
  ...
  return (
    <div class="panel p-2 w-full flex flex-col items-start justify-between">
      <div class="w-full flex flex-col gap-3">
        <div class="mb-2">
          <div class="size-10 grid place-content-center">
            ...
            <span class="text-2xl -rotate-12">⚡︎</span>
            <span class="absolute -top-2 left-1 text-7xl text-blue-500 blur-2xl opacity-80">⚡︎</span>
          </div>
          {!isCollapsed && (
            <div class="px-2">
              <h1 class="mb-1 text-xl font-semibold">Energy Label Generator</h1>
              <p class="text-xs text-neutral-600">
                Powered by{' '}
                <a href={NPM_BETA_URL} target="_blank">
                  energy-label v1.0.0-beta.6
                </a>
              </p>
            </div>
          )}
        </div>
        <Select selectClassName="collapsible" options={flagOptions} isCollapsed={isCollapsed} />
        <div class="py-2">
          <h2 class="mb-1 px-2 text-xs text-neutral-600">Products</h2>
          <nav class="">
            {['Arrow Label', 'Smartphones and Tablets', 'Household refrigerating appliances'].map(template => (
              <button class="py-1 button w-full text-left font-medium hover:bg-neutral-100 truncate">{template}</button>
            ))}
          </nav>
        </div>
      </div>
